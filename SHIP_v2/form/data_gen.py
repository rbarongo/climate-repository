...
#data cols needed
# cc_data = pd.read_excel('Climate Data Template 2.xlsx')
# cc_cols = cc_data.columns
# print(f'Required columns: {len(cc_cols)}\n {cc_cols}')
...
def simulate_bank_data(bank_name, num_records=200):
    ...
    for i in range(num_records):
        loans.append({
            ...
            'Location of invested loan (Region)': random.choice(['Dar es Salaam', 'Arusha', 'Dodoma', 'Mwanza', 'Mbeya', 'Tanga', 'Morogoro', 'Zanzibar Urban/West']),
            ...
            'Collateral Region': random.choice(['Dar es Salaam', 'Arusha', 'Dodoma', 'Mwanza', 'Mbeya', 'Tanga', 'Morogoro', 'Zanzibar Urban/West']),
...
if __name__ == '__main__':
    ...
    for bank in banks:
        ...
        bank_df.to_csv(f'dummy_data/{bank.replace(" ", "_")}_dummyNew.csv', index=False)

        noisy_bank_df = introduce_noise(bank_df)
        noisy_bank_df.to_csv(f'dummy_data/{bank.replace(" ", "_")}_noisy_dummyNew.csv', index=False)
